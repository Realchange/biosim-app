import type { LIFParams as LIFParamsType } from '@biosim/core'
...
const FIELDS: Array<{ key: keyof LIFParamsType; label: string; min: number; max: number; step: number }> = [
  ...
  { key: 'V_threshold', label: 'V_threshold',    min: -70, max: -40, step: 1 },
...
export function LIFParamsPanel({ neuronId, params, studentMode }: Props) {
  ...
  return (
    <>
      <SectionLabel first>{t.lif.stimulus}</SectionLabel>
      <StimParams neuronId={neuronId} params={params} iStimMax={5} />
      <SectionLabel>{t.lif.neuronParams}</SectionLabel>
      {fields.map(f => (
        <label key={f.key} style={{ display: 'block', marginBottom: 8 }}>
          <span style={{ color: '#8b949e', fontSize: 10 }}>{f.key === 'V_threshold' ? t.lif.threshold : f.label}</span>
